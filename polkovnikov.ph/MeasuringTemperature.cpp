...
struct MeasuringTemperature {
	double averageTemperature(vector <int> mv) {
		int s = 0, c = 0;
		ei(a, mv) if (a >= -273) {
			if ((ai <= 0 || abs(mv[ai-1] - a) > 2)
			 && (ai <= 1 || abs(mv[ai-2] - a) > 2)
			 && (ai + 1 >= sz(mv) || abs(mv[ai+1] - a) > 2)
			 && (ai + 2 >= sz(mv) || abs(mv[ai+2] - a) > 2)) continue;
			s += a;
			++c;
		}
		cout << s << ' ' << c << endl;
		return c == 0 ? -300.0 : double(s) / c;
	}
};
...
int main() {
	...
	// ----- test 6 -----
	disabled = false;
	p0 = {-35,-34,-34,-34,-35,72,-34,52,-36,-35,-36,52,-36,-35,981,-33};
	p1 = -34.75;
