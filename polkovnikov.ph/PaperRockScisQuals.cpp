...
struct PaperRockScisQuals {
	int whoPassed(vector <string> p) {
		vector<int> pts(p.size());
		ei(a, p) ei(b, p) if (ai < bi) {
			int x = 0, y = 0;
			ei(c, a) {
				if ("R SP"[c - 'P'] == b[ci]) ++x;
				if ("R SP"[b[ci] - 'P'] == c) ++y;
			}
			if (x == y) ++pts[ai], ++pts[bi];
			else pts[x < y ? bi : ai] += 2;
		}
		return max_element(all(pts)) - pts.begin();
	}
};
...
int main() {
	...
	// ----- test 3 -----
	disabled = false;
	p0 = {"RRRRR","PPPPP","SSSSS","SSSSS","RRRRR","RRRRR"};
	p1 = 0;
	...
	// ----- test 4 -----
	disabled = false;
	p0 = {"PPRPS","RRRPR","SSPRS","SSPRR","PPRSP","SPRPS","SPRSP","PRSPR","PRSPR"};
	p1 = 6;
