...
struct AccessChanger {
	vector <string> convert(vector <string> program) {
		ei(r, program) {
			string s;
			bool f = true, g = true;
			ei(c, r) {
				if (ci > 0 && r[ci-1] == '/' && c == '/') f = false;
				if (f && ci + 1 < sz(r) && c == '-' && r[ci+1] == '>') {
					s += '.';
					g = false;
				} else {
					if (g) s += c;
...
int main() {
	...
	// ----- test 0 -----
	disabled = false;
	p0 = {"Test* t = new Test();","t->a = 1;","t->b = 2;","t->go(); // a=1, b=2 --> a=2, b=3"};
	p1 = {"Test* t = new Test();","t.a = 1;","t.b = 2;","t.go(); // a=1, b=2 --> a=2, b=3"};
	...
	// ----- test 1 -----
	disabled = false;
	p0 = {"---> // the arrow --->","---","> // the parted arrow"};
	p1 = {"--. // the arrow --->","---","> // the parted arrow"};
	...
	// ----- test 2 -----
	disabled = false;
	p0 = {"->-> // two successive arrows ->->"};
	p1 = {".. // two successive arrows ->->"};
