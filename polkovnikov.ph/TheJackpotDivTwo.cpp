...
struct TheJackpotDivTwo {
	vector <int> find(vector <int> money, int jackpot) {
		sort(money);
		int groupSize = 0, groupMoney = 0;
		fori(i, money) {
			int moneyDiff = money[i] - groupMoney;
			if (moneyDiff * groupSize > jackpot)
				break;
			jackpot -= moneyDiff * groupSize;
			groupMoney = money[i];
			++groupSize;
		}
		forn (i, groupSize)
			money[i] = groupMoney + jackpot / groupSize + (i < jackpot % groupSize);
		sort(money);
		return money;
	}
};
