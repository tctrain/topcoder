...
struct Spamatronic {
    ...
    static void update(set<string> & b, string const & s) {
        ...
        for(typeof(r.begin())i=r.begin(),ie=r.end();i!=ie;++i) b.insert(*i);
    }
    bool isspam(set<string> const & b, string const & s) {
        set<string> r = towords(s);
        int c = 0;
        for(typeof(r.begin())i=r.begin(),ie=r.end();i!=ie;++i)
            if (b.find(*i) != b.end())
                ++c;
    ...
    vector <int> filter(vector <string> knownSpam, vector <string> newMail) {
        set<string> base;
        for(int i=0,ie=((knownSpam).size());i<ie;++i) update(base, knownSpam[i]);
        vector<int> r;
        for(int i=0,ie=((newMail).size());i<ie;++i) {
