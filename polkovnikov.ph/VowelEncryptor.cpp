...
struct VowelEncryptor {
	vector <string> encrypt(vector <string> text) {
		ei(a, text) {
			int c = 0;
			ei(b, a) if (string("aeiou").find(b) != string::npos) ++c;
			if (c == sz(a)) continue;
			string r;
			ei(b, a) if (string("aeiou").find(b) == string::npos) r += b;
			a = r;
		}
		return text;
	}
...
int main() {
	...
	// ----- test 2 -----
	disabled = false;
	p0 = {"he","who","is","greedy","is","disgraced"};
	p1 = {"h","wh","s","grdy","s","dsgrcd"};
