...
struct NounReform {
	vector <string> makePlural(vector <string> nouns) {
		...
		ei(a, nouns) {
			...
			if (a.size() >= 2 && string("aeiou").find(a[a.size()-2]) != string::npos && a.back() == 'y') {
				a += "s";
				continue;
			}
			if (a.back() == 'y') {
...
int main() {
	...
	// ----- test 0 -----
	disabled = false;
	p0 = {"box","church","elephant","stereo"};
	p1 = {"boxes","churches","elephants","stereos"};
	...
	// ----- test 1 -----
	disabled = false;
	p0 = {"tray","key","enemy","baby"};
	p1 = {"trays","keys","enemies","babies"};
