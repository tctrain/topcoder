...
struct InverseFactoring {
	int getTheNumber(vector <int> x) {
		map<int, int> c;
		ei(a, x) {
			int aa = a;
			map<int, int> z;
			...
			if (aa > 1) ++c[aa];
			ei(a, z) smx(c[a.first], a.second);
		}
		int m = 1;
		ei(a, c) fr(i, a.second) m *= a.first;
		return m;
	}
};
...
int main() {
	...
	// ----- test 3 -----
	disabled = false;
	p0 = {14,26456,2,28,13228,3307,7,23149,8,6614,46298,56,4,92596};
	p1 = 185192;
