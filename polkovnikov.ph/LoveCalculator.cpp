...
struct LoveCalculator {
	string findBoy(string girl, vector <string> boys) {
		...
		int c[4] = {};
		ei(a, girl) { int pos = string("LOVE").find(a); if (pos != string::npos) ++c[pos]; }
		ei(boy, boys) {
			int d[4] = {};
			ei(a, boy) { int pos = string("LOVE").find(a); if (pos != string::npos) ++d[pos]; }
			int s = 1;
			fr(i, 4) fr(j, 4) if (i < j) {
				s *= c[i] + d[i] + c[j] + d[j];
			}
			s %= 100;
			v.eb(-s, boy);
...
int main() {
	...
	disabled = false;
	p0 = "LLOL";
	p1 = {"BVERON","CVERON","AVERON","DVERON"};
	p2 = "AVERON";
	...
	disabled = false;
	p0 = "VELYLEOCEVE";
	p1 = {"YVXHOVE","LCOKO","OGWSJVEVEDLE","WGFVSJEL","VLOLUVCBLLQVESWHEEKC"};
