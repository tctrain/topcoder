...
struct WordAbbreviation {
	vector <string> getAbbreviations(vector <string> words) {
		ei(w, words) {
			ei(a, w) {
				string p = w.substr(0, ai + 1);
				bool f = true;
				ei(h, words) if (hi != wi) {
					if (sz(h) >= sz(p) && h.substr(0, ai + 1) == p) {
						f = false;
						break;
					}
				}
...
int main() {
	...
	// ----- test 3 -----
	disabled = false;
	p0 = {"bababaaaaa","baaabaababa","bbabaaabbaaabbabaabaabbbbbaabb","aaababababbbbababbbaabaaaaaaaabbabbbaaab","baaaaabaababbbaabbbabbababbbabbbbbbbbab"};
	p1 = {"bab","baaab","bb","a","baaaa"};
